...
int main()
{
  ...
  do
  {
    printf("\nEnter a choice\n0. Exit\n1. Add\n2. Display\n3. Search a contact\n4. Delete contact\n5. Update Contact\n6. Export Contacts to the txt file\n7. Import Contacts from the txt file\n8. Clear screen\n");
    scanf("%d", &option);
    switch (option)
    {
    case 0:
    ...
    case 6:
      fopen_s(&fp, "ContactsExport.txt", "w");
      if (fp == NULL)
      {
        printf("Error opening file");
        exit(1);
      }
      struct Contact_book *cba = head;
      char num[20];
      sprintf(num, "%lld", cba->number);
      while (cba != NULL)
      {
        fprintf(fp, "Name: %s\n", cba->name);
        fprintf(fp, "Number: %s\n", num);
      ...
      break;
    case 7:
      fopen_s(&fp, "ContactsImport.txt", "r");
      if (fp == NULL)
      {
        printf("Error opening file");
        exit(1);
      }
      while (fscanf(fp, "%s %lld %s", name, &number, email) == 3)
      {
        add_to_list(p, number, name, email);
      }
      fclose(fp);
